...
export function estimateReadTimeMinutes(...parts: Array<string | null | undefined>) {
  const text = parts
...
export async function readStoryFromSearchParams(
  ...
  const readTimeParam = Number.parseInt(firstValue(resolved.readTimeMinutes) ?? "", 10)
...
export function extractTopics(items: Array<Pick<NewsItem, "title" | "description">>) {
...
export function getNewsPulse(items: NewsItem[]) {
  ...
  const averageReadTime = items.length
    ? Math.max(
        1,
        Math.round(
          items.reduce((total, item) => total + item.readTimeMinutes, 0) / items.length,
        ),
      )
    : 0
